<div @class([
    ...
    <a @class(['group flex flex-col gap-3 items-center '])
        @if ($order->status == 'kot' && !is_null($order->table_id)) href="{{ route('pos.order', $order->table_id) }}" wire:navigate
       @elseif ($order->status == 'kot' && is_null($order->table_id))
           href="{{ route('pos.kot', $order->id).'?show-order-detail=true' }}" wire:navigate
       @else
           wire:click="$dispatch('showOrderDetail', { id: {{ $order->id }} })" @endif
        ...
        <div class="flex w-full justify-between items-center">
            @if ($order->order_type == 'pickup')
                <div class="flex flex-col text-xs text-gray-600 dark:text-gray-400">
                    <div>
                        <span>@lang('modules.order.pickupDate')</span>
                        {{ \Illuminate\Support\Carbon::parse($order->pickup_date)->translatedFormat('F d, Y h:i A') }}
                    </div>
                    <div>
                        <span>@lang('modules.order.orderDate')</span>
